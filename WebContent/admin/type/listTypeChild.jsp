...
<body>
	...
	<div class="crumb-wrap">
		<div class="crumb-list">
			...
			<span class="crumb-step">&gt;</span><span class="crumb-name">子版区管理</span>
		</div>
	</div>
	...
	<div class="result-wrap">
		<div class="result-title">
			<div class="result-list">
				<a href="addTypeChild.jsp"><i class="icon-font"></i>新增子版区</a> <a id="batchDel"
		...
		<div class="result-content">
			<table class="result-tab" width="100%">
				...
				<!-- forEach 遍历出 tychList -->
				<c:forEach items="${tychList }" var="item" varStatus="status">
					<tr>
						<td class="tc"><input name="id[]" value="59" type="checkbox"></td>
						<td>${item.tychid}</td>
						<td>${typeBean.name}</td>					
						<td>${item.name}</td>
						<td><a href="#" >${item.info}</a></td>
						<td><a class="link-update" href="${pageContext.request.contextPath }/admin/type/typeServlet?method=updTypeChild&tychid=${item.tychid}&parentid=${typeBean.typeid}">修改</a> <a
							class="link-del" href="${pageContext.request.contextPath }/admin/type/typeServlet?method=delTypeChild&tychid=${item.tychid}">删除</a></td>
					</tr>
				</c:forEach>
			</table>
			...
			<c:if test="${param.status.equals('1') }">
				<div class="alert alert-success" role="alert">请先删除该版区下的帖子</div>
			</c:if>
		</div>
	</div>
</body>
